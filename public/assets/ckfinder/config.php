...
function CheckAuthentication()
{
	...
	if (isset($_SESSION["admin"]["allow_ckfinder"]) &&  $_SESSION["admin"]["allow_ckfinder"] === true)
	{
		return true;
	}
...
/*
DCMS
Set the size of uploaded images to generate an extra file. Scale to the given properties
and the file goes to the given dir
please make sure you have the folder available
*/
$config['DCMSScaleCrop'] = Array('Scale' => array(
											'plant_320x180' => array(
															//define which directories may automaticly generate large subfolder
															'alloweddirectoriesexactmatch' => false,//false ? the task is executed for the subdir as well
															'alloweddirectories' => array(strtolower(str_replace("\\","/",str_replace("//","/",$baseDir))).'image/hobby/plant/'),
															//'dir'=> $baseDir.'/images/large/',
															'folderpath'=> '',
															'subfolder'=> '',
															'width'=>'320',
															'height'=>'180',
															'filename_postfix' => '_320x180', // string  to attach at the end of the filename e.g. filename = "myimage.jpg", filename_postfix ="_100x200" ==> filename =  myimage_100x200.jpg"
															),
											),
								);
...
// Hide our custom folders
//------------------------------
$config['AccessControl'][] = Array(
		'role' => '*',
		'resourceType' => 'Images',
		'folder' => '/articles/large',
		'folderView' => false);

$config['AccessControl'][] = Array(
		'role' => '*',
		'resourceType' => 'Images',
		'folder' => '/articles/medium',
		'folderView' => false);

$config['AccessControl'][] = Array(
		'role' => '*',
		'resourceType' => 'Images',
		'folder' => '/articles/small',
		'folderView' => false);

$config['AccessControl'][] = Array(
		'role' => '*',
		'resourceType' => 'Images',
		'folder' => '/products/large',
		'folderView' => false);
...
$config['AccessControl'][] = Array(
		'role' => '*',
		'resourceType' => 'Images',
		'folder' => '/products/small',
		'folderView' => false);

//-------------- end hiding custom folders --------------
...
include_once "plugins/DCMSScaleCrop/plugin.php";
